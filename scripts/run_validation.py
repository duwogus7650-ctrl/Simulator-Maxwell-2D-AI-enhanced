...
def run():
    ...
    if os.path.exists(p):
        ...
        # ⚠ 효율은 턴수 의존: 14턴서 motoropt 74%(동손 399W) vs 보고서 67.8%
        # (동손 533W). MLT 동손모델이 KRO80에서 ~33% 과소 — 별도 검증과제.
        # 기준은 motoropt 실측(드리프트 감지용), 정확도 격차는 주석으로 명시.
        checks.append(("KRO80 효율@순시(14턴)", eff, 0.741, 0.02, "abs",
                       "보고서 67.8%와 +6%p 차이(동손모델 과소·검증과제)"))
    else:

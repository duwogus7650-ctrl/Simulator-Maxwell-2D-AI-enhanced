...
for k, v in metrics.items():
    print(f"{k:12s} CV R²={v['R2_cv']:.3f}  홀드아웃={v['R2_holdout']:.3f}  "
          f"MAE={v['MAE']:.3g}  rel={v['rel%']:.1f}%  "
          f"{'신뢰' if v['reliable'] else '⚠노이즈'}")
_rel = [k for k in Y_KEYS if metrics[k]["reliable"]]
save(model, scale, "surrogate.joblib", y_keys=Y_KEYS, reliable_keys=_rel)
...
for j, (ax, k) in enumerate(zip(axes, Y_KEYS)):
    ...
    ax.set_title(f"{k} (홀드아웃 R²={metrics[k]['R2_holdout']:.3f})")
    ax.grid(alpha=.3)

...
def sweep_load_with_fields(model: dict, style: str, *,
    ...
    for i in range(n_steps):
        ...
        # NOTE: 스윕 샘플 coenergy로 중앙차분하는 방안은 본 스윕이 각도와
        # 전류를 동시에 진행시켜(전류 비고정) 1전기주기에 걸쳐 W'(θ)가
        # 거의 주기적 → 차분 평균이 0에 수렴해 물리적으로 무의미하다
        # (검증: 중앙차분 T_avg≈0 vs Arkkio −0.92 N·m). 고정전류 가상일을
        # 얻으려면 스텝마다 별도 솔브가 필요하므로 전진차분을 유지한다.
        # 단, 전진차분은 O(dθ) 바이어스가 있다(dθ=0.25° 기계각).
